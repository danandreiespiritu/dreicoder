   ...
   <!-- Dashboard content -->
        <main class="p-4 sm:p-6 space-y-6 overflow-auto">
            <div class="flex flex-col items-center justify-center min-h-[70vh]">
                <div class="w-full max-w-lg bg-white p-4 sm:p-6 md:p-8 rounded-lg shadow-lg">
                    <form method="POST" action="{{ route('admin.brand.store') }}" enctype="multipart/form-data" class="w-full">
                        @csrf
                        <h1 class="text-2xl md:text-3xl font-bold mb-6 text-gray-800 text-center">Add Brand</h1>

                        <div class="mb-4">
                            <label for="productBrandName" class="block text-sm font-medium text-gray-700 mb-2">Category Name</label>
                            <input type="text" name="productBrandName" id="productBrandName" required
                        ...
                        <button type="submit"
                            class="w-full bg-indigo-600 text-white py-2 md:py-3 rounded-lg font-medium hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition duration-300 text-sm md:text-base">
                            Submit
                        </button>

                        @if(session('success'))
                            <div class="bg-green-500 text-white p-4 rounded-lg mt-4 text-center text-sm">
                                {{ session('success') }}
                            </div>
                        @endif
                    </form>
